...
È possibile visualizzare le impostazioni di rete per un nodo di archiviazione nella pagina *Impostazioni di rete* quando si accede al nodo (https://<node[] IP>:442/hcc/node/network-settings). È possibile selezionare le impostazioni *Bond1G* (gestione) o *Bond10G* (storage). L'elenco seguente descrive le impostazioni che è possibile modificare quando un nodo di storage si trova nello stato Available (disponibile), Pending (in sospeso) o Active (attivo):
