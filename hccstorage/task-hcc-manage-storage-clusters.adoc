...
.Di cosa hai bisogno
* *Autorizzazioni di amministratore del cluster*: Si dispone delle autorizzazioni di amministratore su link:../concepts/concept_intro_clusters.html#authoritative-storage-clusters["cluster di storage autorevole"]. Il cluster autorevole è il primo cluster aggiunto all'inventario dei nodi di gestione durante l'installazione del sistema.
...
Se devi portare un nodo storage offline per manutenzione, come ad esempio aggiornamenti software o riparazioni host, puoi ridurre al minimo l'impatto di i/o per il resto del cluster storage in base alla modalità di manutenzione per <<enable_main_mode,abilitazione in corso>>quel nodo. Quando si utilizza <<disable_main_mode,disattiva>>la modalità di manutenzione, il nodo viene monitorato per garantire che vengano soddisfatti determinati criteri prima che il nodo possa uscire dalla modalità di manutenzione.
