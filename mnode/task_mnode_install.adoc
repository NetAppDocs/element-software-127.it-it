...
.. Aggiungi una nuova riga per ogni server orario predefinito che desideri aggiungere. I server orari predefiniti devono essere gli stessi server NTP utilizzati sul cluster di storage che utilizzerai in un <<set-up-the-management-node,passo successivo>>.
+
[listing]
----
vi /etc/ntp.conf
...
*** *--storage_mvip (-sm) [indirizzo MVIP]*: L'indirizzo IP virtuale di gestione (MVIP) del cluster di storage su cui è in esecuzione il software Element. Configura il nodo di gestione con lo stesso cluster di storage che hai utilizzato durante link:task_mnode_recover.html#configure-time-sync["Configurazione dei server NTP"].
