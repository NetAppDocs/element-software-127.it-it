...
[role="lead"]
L'oggetto snmpTrapRecipient contiene informazioni su un host configurato per ricevere trap SNMP generati dal cluster di storage. È possibile utilizzare il `GetSnmpTrapInfo` metodo API per ottenere un elenco di host configurati per ricevere trap SNMP.
