...
[role="lead"]
L'oggetto volumePair contiene informazioni su un volume associato a un altro volume su un cluster diverso. Se il volume non è associato, l'oggetto è vuoto. È possibile utilizzare i `ListActivePairedVolumes` metodi e `ListActiveVolumes` API per restituire informazioni sui volumi accoppiati.
