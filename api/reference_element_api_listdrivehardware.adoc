...
[role="lead"]
È possibile utilizzare il `ListDriveHardware` metodo per elencare tutte le unità collegate a un nodo. Se utilizzato su singoli nodi, questo metodo restituisce informazioni sull'hardware del disco. Se utilizzato sul nodo master del cluster MVIP, questo metodo restituisce informazioni per tutti i dischi su tutti i nodi.
