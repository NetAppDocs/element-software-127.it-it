...
[role="lead"]
È possibile utilizzare il `GetHardwareConfig` metodo per ottenere le informazioni di configurazione hardware per un nodo. Questi dati di configurazione sono destinati all'uso interno. Per ottenere un inventario più utile dei componenti hardware del sistema in tempo reale, utilizzare il `GetHardwareInfo` metodo.
