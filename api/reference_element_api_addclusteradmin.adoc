...
[role="lead"]
È possibile utilizzare il `AddClusterAdmin` metodo per aggiungere un nuovo account amministratore cluster. Un amministratore del cluster può gestire il cluster tramite API e strumenti di gestione. Gli amministratori dei cluster sono completamente separati e non correlati agli account dei tenant standard.
