...
[role="lead"]
L'oggetto pendingNode contiene informazioni su un nodo che può essere aggiunto a un cluster. Utilizzare il `ListPendingNodes` metodo API per restituire un elenco di queste informazioni per tutti i nodi in sospeso. È possibile aggiungere uno qualsiasi dei nodi elencati a un cluster utilizzando il `AddNodes` metodo API.
