...
[role="lead"]
L'oggetto Network (Local Interfaces) contiene informazioni di configurazione per le interfacce di rete locali, come l'interfaccia di loopback, su un nodo di storage. È possibile utilizzare i `GetConfig` metodi e `GetNetworkConfig` per ottenere queste informazioni per un nodo di storage.
