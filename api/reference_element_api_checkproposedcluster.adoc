...
[role="lead"]
È possibile utilizzare questo `CheckProposedCluster` metodo per testare una serie di nodi di storage prima di creare un cluster di storage con loro per identificare possibili errori o guasti che potrebbero verificarsi nel tentativo, ad esempio funzionalità di nodi misti non bilanciate o tipi di nodi non supportati per cluster di storage a due nodi.
