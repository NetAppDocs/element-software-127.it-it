...
[role="lead"]
È possibile utilizzare il `SetNetworkConfig` metodo per impostare la configurazione di rete per un nodo. Per visualizzare le impostazioni di rete correnti per un nodo, eseguire il `GetNetworkConfig` metodo API.
