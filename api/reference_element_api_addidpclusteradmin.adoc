...
[role="lead"]
È possibile utilizzare il `AddIpdClusterAdmin` metodo per aggiungere un utente amministratore cluster autenticato da un provider di identità di terze parti (IdP). Gli account di amministrazione del cluster IDP sono configurati in base alle informazioni sul valore dell'attributo SAML fornite nell'asserzione SAML di IdP associata all'utente. Se un utente esegue correttamente l'autenticazione con IdP e dispone di istruzioni degli attributi SAML all'interno dell'asserzione SAML corrispondenti a più account di amministrazione del cluster IdP, l'utente disporrà del livello di accesso combinato di quelli corrispondenti agli account di amministrazione del cluster IdP.
...
nome utente
 a| 
Mapping attributo-valore SAML a un amministratore del cluster IdP (ad esempio, email=test@example.com). Questo può essere definito utilizzando un oggetto SAML specifico utilizzando `NameID` o come voce nell'istruzione SAML Attribute, ad esempio `eduPersonAffiliation` .
 a| 
stringa
 a| 
Nessuno
