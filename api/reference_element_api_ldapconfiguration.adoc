...
[role="lead"]
L'oggetto ldapConfiguration contiene informazioni sulla configurazione LDAP nel sistema di storage. È possibile recuperare le informazioni LDAP con il `GetLdapConfiguration` metodo API.
...
Server URI
 a| 
Elenco separato da virgole di URI server LDAP (ad esempio, `ldap://1.2.3.4` e `ldaps://1.2.3.4:123`).
 a| 
stringa
