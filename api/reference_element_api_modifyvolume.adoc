...
[role="lead"]
È possibile utilizzare il `ModifyVolume` metodo per modificare le impostazioni di un volume esistente. È possibile apportare modifiche a un volume alla volta e le modifiche vengono eseguite immediatamente.

Se non si specificano i valori QoS quando si modifica un volume, questi rimangono gli stessi di prima della modifica. È possibile recuperare i valori QoS predefiniti per un volume appena creato eseguendo il `GetDefaultQoS` metodo.
...
* `replicationTarget`: Consente di identificare un volume come volume di destinazione per un set di volumi associati. Se il volume non è associato, lo stato di accesso è bloccato. Se non viene specificato alcun valore, il valore di accesso non cambia.
* `snapMirrorTarget`: Consente di identificare un volume come volume di destinazione per la replica SnapMirror.
...
* `asynch`: Attende che il sistema riconosca che i dati sono memorizzati sulla sorgente prima di scrivere sulla destinazione.
