...
[role="lead"]
È possibile utilizzare il `TestConnectEnsemble` metodo per verificare la connettività con un gruppo di database specificato. Per impostazione predefinita, utilizza l'insieme per il cluster a cui è associato il nodo. In alternativa, è possibile fornire un gruppo diverso per testare la connettività.
...
* `nodes`: (Oggetto) Un elenco di ogni nodo ensemble nel test e i risultati dei test.
