...
[role="lead"]
L'oggetto rete (interfacce Ethernet) contiene informazioni di configurazione per singole interfacce Ethernet. È possibile utilizzare i `GetConfig` metodi e `GetNetworkConfig` per ottenere queste informazioni per un nodo di storage.
