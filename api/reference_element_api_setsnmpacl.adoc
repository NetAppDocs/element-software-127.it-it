...
[role="lead"]
È possibile utilizzare il `SetSnmpACL` metodo per configurare le autorizzazioni di accesso SNMP sui nodi del cluster. I valori impostati con questa interfaccia si applicano a tutti i nodi nel cluster e i valori passati sostituiscono tutti i valori impostati in qualsiasi chiamata precedente a `SetSnmpACL`. Si noti inoltre che i valori impostati con questa interfaccia sostituiscono tutti i valori di rete o usmUsers impostati con il `SetSnmpInfo` metodo.
