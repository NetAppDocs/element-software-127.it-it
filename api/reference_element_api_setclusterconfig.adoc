...
[role="lead"]
È possibile utilizzare il `SetClusterConfig` metodo per impostare la configurazione utilizzata da un nodo per comunicare con il cluster a cui è associato. Per visualizzare le impostazioni correnti dell'interfaccia cluster per un nodo, eseguire il `GetClusterConfig` metodo API.
