...
[role="lead"]
È possibile utilizzare questo `ModifyVolumes` metodo per configurare fino a 500 volumi esistenti alla volta. Le modifiche vengono eseguite immediatamente. Se `ModifyVolumes` non è possibile modificare uno dei volumi specificati, nessuno dei volumi specificati viene modificato.

Se non si specificano i valori QoS quando si modificano i volumi, i valori QoS per ciascun volume rimangono invariati. È possibile recuperare i valori QoS predefiniti per un volume appena creato eseguendo il `GetDefaultQoS` metodo.
